...
docker-compose -p ${PROJECT} run --entrypoint /bin/bash addok-fr -c "bzcat data/BAN_odbl.json.bz2 | jq -c 'del(.housenumbers[]?.id)' | addok batch"

docker-compose -p ${PROJECT} run --entrypoint /bin/bash addok-fr -c "ls data/*.json | xargs cat | addok batch"
